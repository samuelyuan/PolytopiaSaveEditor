...
func BuildNewPlayerUnknownArr(oldRelationArr []PlayerRelationData, newPlayerId int) []PlayerRelationData {
...
func ModifyAllExistingPlayerUnknownArr(inputFilename string) {
	...
	for i := len(saveOutput.PlayerData) - 1; i >= 0; i-- {
		...
		newRelationArr := BuildNewPlayerUnknownArr(saveOutput.PlayerData[i].RelationArr, newPlayerId)
